...
public class GlobalInferenceView extends ControlFlowView {
	...
	private Map<String, Set<FsmStateIncoherence>> fEntryIncoherences; // map (entry id, incoherent events)
	
	public GlobalInferenceView() {
		super();
		
		fEntryIncoherences = new HashMap<>();
	}
	
	public void setProperties(Map<String, Set<FsmStateIncoherence>> pEntries) {
		fEntryIncoherences.putAll(pEntries);
	}
	...
	protected List<ITimeEvent> createTimeEvents(ControlFlowEntry controlFlowEntry, Collection<ITmfStateInterval> value) {
		...
		Map<TmfXmlFsmTransition, ITmfEvent> inferredEvents = new HashMap<>();
		
		Set<FsmStateIncoherence> incoherences = fEntryIncoherences.get(String.valueOf(controlFlowEntry.getThreadId()));
		if (incoherences != null) {
			for (FsmStateIncoherence incoherence : incoherences) {
				inferredTransitions.addAll(incoherence.getInferredTransitions());
				inferredEvents.putAll(incoherence.getInferredEventsMap());
			}
		}
		
		// Get iterator on inferred transitions
		if (!inferredTransitions.isEmpty()) {
	        ...
	        if (it.hasNext()) {
	        	transition = it.next();
	        	inferredEvent = inferredEvents.get(transition);
